var pg  = require('pg');
var conString = "postgres://localhost:5432/sendatask";
...
function User(name,username,email,password){
    ...
    this.password= password; //need to declare the things that i want to be remembered for each user in the database

    console.log("Inside new user for :  " + this.name);
    this.save = function() {
        var conString = "postgres://localhost:5432/sendatask";
        ...
        console.log(this.email +' will be saved');
            ...
            client.query('INSERT INTO users(name, username,email,password) values($1,$2,$3,$4) ', [this.name, this.username, this.email, this.password], function (err, result) {
                if(err){
                    console.log(err);
            ...
            // client.query('SELECT * FROM users ORDER BY username desc limit 1', null, function(err, result){
